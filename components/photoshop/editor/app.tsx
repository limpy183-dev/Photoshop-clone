...
import { buildLearningIndex, runLearningIndexItem } from "@/editor/learning-index"
import { findNewDocumentPreset } from "@/editor/new-document-presets"
import { readRecentDocuments, rememberRecentDocument } from "@/editor/recent-documents"
import { createDocumentFromPreset, createDocumentFromRasterImport } from "@/editor/startup-documents"
import {
  deleteStartupImageImport,
  readStartupImageImport,
  STARTUP_IMAGE_IMPORT_PARAM,
} from "@/editor/startup-file-handoff"
import {
  applyScreenMode,
  cycleScreenMode,
...
function StartupRouteEffects() {
  ...
  React.useEffect(() => {
    ...
    const openRecent = async () => {
      ...
      if (!recent) return
      try {
        const { deserializeProject } = await import("@/editor/document/io")
    ...
    const openStartupImport = async () => {
      ...
      try {
        const {
          ...
          serializeProject,
        } = await import("@/editor/document/io")
