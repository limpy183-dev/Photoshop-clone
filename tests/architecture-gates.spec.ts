...
test("architecture gate reports no import cycles or budget regressions", () => {
  ...
  // 8 -> 16 when subsystems-dialog.tsx became one file per tab, then 16 -> 21 when
  // adjustment-dialogs.tsx did the same and the preset/brush/actions panels shed
  // siblings. This budget counts FILES, so components that already called
  // useEditor() from inside a shared file now each count once. Call sites were
  // verified unchanged across both passes (9 and 7 respectively), so nothing new
  // subscribes to whole-editor state. Raise this only for that same reason; a
  // genuinely new useEditor() consumer should still be pushed to useEditorSelector.
  expect(report.budgets.useEditorImports.max).toBeLessThanOrEqual(21)
