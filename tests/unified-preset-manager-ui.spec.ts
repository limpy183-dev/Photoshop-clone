...
test("unified preset manager opens as one cross-family panel", async ({ page }) => {
  ...
  await expect(page.getByLabel("Preset set", { exact: true })).toBeVisible()
  ...
  await expect(page.getByLabel("Preset family")).toHaveValue("brush")
  await expect(page.getByRole("button", { name: /Soft Round 30 Brushes - General/ })).toBeVisible()
})
